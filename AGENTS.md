...
| Volume Vue `import()` of `/api/plugins/.../entry.js` | Native import maps do **not** apply to that module URL. Fetch the bundle, rewrite known bare specifiers onto **absolute** `origin/khirby-peers/*.js` URLs (blob modules cannot resolve `/khirby-peers/…`), then `import(blobUrl)`. Vue/router/i18n shims must re-export compiler helpers (`withModifiers`, …), not a hand-picked subset. Do not treat a matching `index.html` import-map key as proof — open `/plugins/listmonk` in the browser |
| Volume Nest `ts-node` + class-validator DTOs | `loadVolumeNestModuleFile` must register ts-node with `useDefineForClassFields: false` (and prefer `target: ES2021`), and must **purge `require.cache` for the plugin dir** before loading — `createPlugin()` via jiti often imports Nest controllers first and freezes bad class fields. Do not `import` the Nest module from the jiti-loaded plugin entry (AI Compose pattern: let the host attach it). Prefer plain `@Body()` types over class-validator DTO classes in volume plugins |
