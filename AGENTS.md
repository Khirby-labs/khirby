...
| Volume Nest `ts-node` + class-validator DTOs | `loadVolumeNestModuleFile` must register ts-node with `useDefineForClassFields: false` (and prefer `target: ES2021`). Without it, TS 5 emits `Object.defineProperty` for class fields and ValidationPipe dies with `Decorating class property failed` → Listmonk preview/lookup 500. Do not “fix” by dropping `@Is*` from every plugin DTO — fix the host register options in `packages/plugin-host/src/volume-nest.ts` |
