...
| `KHIRBY_SECRETS_KEY` | 32-byte hex key — encrypts mailbox passwords, Google refresh tokens, and plugin secrets at rest (ADR-0046) |
| `MAIL_SECRETS_KEY` / `AI_COMPOSE_SECRETS_KEY` / `POKELO_SECRETS_KEY` | Legacy aliases for the same AES key. Keep until ciphertext is rewritten under `KHIRBY_SECRETS_KEY`; do not add new per-plugin names |
| `GOOGLE_MAIL_CLIENT_ID` / `GOOGLE_MAIL_CLIENT_SECRET` | Optional — Gmail/Workspace OAuth |
| `CONTROL_PLANE_URL` | Optional override of Bearly Control Plane (no trailing slash). **Unset defaults to `https://ctrl.bearly.pro`** (ADR-0051). **Explicit empty = no outbound Control Plane calls** — marketplace catalog stays empty, heartbeats soft-fail, register returns an error (ADR-0044) |
| `DISABLE_TELEMETRY` | Optional. Set to `true`, `1`, or `yes` to disable anonymous installation heartbeats entirely. Catalog / register / submit still use `CONTROL_PLANE_URL` or the `https://ctrl.bearly.pro` default (ADR-0044, ADR-0051). Production: at most one heartbeat per 24h. Non-production (`NODE_ENV` ≠ `production`): heartbeat on every process start (no 24h gate; ~5s boot delay). Payload is counts and feature flags only — no emails, names, or message bodies |
...
4. Env: `GOOGLE_MAIL_CLIENT_ID`, `GOOGLE_MAIL_CLIENT_SECRET`, `KHIRBY_SECRETS_KEY`
...
services:
  ...
  app:
    ...
    environment:
      ...
      KHIRBY_SECRETS_KEY: ${KHIRBY_SECRETS_KEY:-}
