export const meta = {
  name: 'review-panel',
  description:
    'Blind 3-perspective review of a diff; every finding adversarially verified before it counts',
    ...
    findings: {
      ...
      items: {
        ...
        properties: {
          ...
          repro_hint: {
            type: 'string',
            description: 'how a verifier could confirm this (command, scenario)',
          },
        },
      },
    },
...
const VERDICT_SCHEMA = {
  ...
  properties: {
    ...
    evidence: {
      type: 'string',
      description: 'repro output / code citation, or the reason the finding is refuted',
    },
  },
};
  ...
  PERSPECTIVES.map(
    (p) => () =>
      agent(p.prompt, {
        ...
        model: p.model,
      }).then((r) => (r ? r.findings.map((f) => ({ ...f, source: `panel/${p.key}` })) : [])),
  ),
);
...
if (!unique.length) {
  return {
    ...
    rejected: [],
    ledgerRows: [],
    note: 'PANEL CLEAN — no findings from any perspective.',
  };
}
...
return {
  ...
  ledgerRows: confirmed.map(
    (f) =>
      `| ${f.id} | ${f.source} | ${f.severity} | ${f.desc} (${f.file}:${f.line}) | CONFIRMED | ${(f.evidence || '').replace(/\|/g, '/').slice(0, 120)} |`,
  ),
};
