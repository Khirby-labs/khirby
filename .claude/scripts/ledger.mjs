...
//   - loop budgets (critique 2 · audit 3 · panel 2),
...
//   node .claude/scripts/ledger.mjs budget <plan-file> <loop>   count one cycle of critique|audit|panel; exit 1 when exhausted
import { readFileSync, writeFileSync, existsSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { resolve } from 'node:path';

const STATES = ['OPEN', 'CONFIRMED', 'FIXED', 'VERIFIED', 'REJECTED'];
const BUDGETS = { critique: 2, audit: 3, panel: 2 };
...
if (!cmd || !planArg || (cmd === 'budget' && !(loop in BUDGETS))) {
  console.log('usage: ledger.mjs sync|open <plan-file> | ledger.mjs budget <plan-file> critique|audit|panel');
  process.exit(2);
}
...
function parseLedger() {
  ...
  if (section === undefined) {
    console.error('[ledger] no "## Findings ledger" section in the plan file — add it (see plans/template.md).');
    process.exit(2);
  }
  const rows = [];
...
function validate(rows) {
  ...
  for (const r of rows) {
    ...
    seen.add(r.id);
    if (!STATES.includes(r.status)) errors.push(`${r.id}: illegal state "${r.status}" (allowed: ${STATES.join(' ')})`);
...
const entry = all[key] ?? { findings: {}, budgets: { critique: 0, audit: 0, panel: 0 } };
...
if (cmd === 'budget') {
  ...
  if (used > max) {
    console.log(`[ledger] ${loop} budget EXHAUSTED (${used - 1}/${max} cycles used). STOP — escalate to the human with the open findings; do not run another cycle.`);
    process.exit(1);
  }
...
if (cmd === 'open') {
  ...
  if (!open.length) console.log('[ledger] no CONFIRMED findings — nothing may loop back to the implementer.');
...
for (const [id, prev] of Object.entries(entry.findings)) {
  ...
  if (prev === 'VERIFIED' && (!now || now.status !== 'VERIFIED')) {
    console.error(`[ledger] OSCILLATION: ${id} was VERIFIED and is now ${now ? now.status : 'missing'}. A fix reverted verified work.`);
    oscillation = true;
  }
}
...
console.log(`[ledger] ${rows.length} findings: ${STATES.map((s) => `${s}=${counts[s] ?? 0}`).join(' ')}`);
if (oscillation) {
